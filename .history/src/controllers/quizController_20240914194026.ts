...
export const addQuestionToQuiz = async (req: Request, res: Response) => {
  ...
  if (quiz) {
    quiz.questions.push(question._id as Types.ObjectId as unknown as Schema.Types.ObjectId);
    await quiz.save();
...
export const addQuestionsToQuiz = async (req: Request, res: Response) => {
  ...
  const quiz = await Quiz.findById(req.params.quizId);
  if (quiz) {
    questions.forEach(question => quiz.questions.push(question._id as Types.ObjectId as unknown as Schema.Types.ObjectId));
    await quiz.save();
